...
if [ $(get_opt build $@) ]; then
	_previous=$(docker images --filter "label=root@${version}" | grep ${version})
	echo_info "pre-build image: ${_previous}"
	_previous=$(docker images --filter "label=root@${version}" | grep ${version} | tr -s ' ' | cut -f3 -d' ')
	echo_info "pre-build image id: ${_previous}"
	docker build -t ${docker_image_local} --label "root@${version}" -f ${THISD}/Dockerfile ${PWD}
	nimages=$(docker images --filter "label=root@${version}" | grep ${version} | wc -l)
	docker images --filter "label=root@${version}"
	if [ ! -z ${_previous} ]; then
		echo_info "pre-build image id: ${_previous}"
