...
package_name="pyjetty"
version=$(cat ${THISD}/Dockerfile | grep FROM | grep heppy | cut -d':' -f2)
echo_info "version is: ${version}"
docker_image_local=${package_name}:${version}

### --build
if [ $(get_opt build $@) ]; then
	_previous=$(docker images --filter "label=pyjetty@${version}" | grep ${version})
	echo_info "pre-build image: ${_previous}"
	_previous=$(docker images --filter "label=pyjetty@${version}" | grep ${version} | tr -s ' ' | cut -f3 -d' ')
	echo_info "pre-build image id: ${_previous}"
	docker build -t ${docker_image_local} --label "pyjetty@${version}" -f ${THISD}/Dockerfile ${PWD}
	nimages=$(docker images --filter "label=pyjetty@${version}" | grep ${version} | wc -l)
	docker images --filter "label=pyjetty@${version}"
	if [ ! -z ${_previous} ]; then
		echo_info "pre-build image id: ${_previous}"
...
if [ $(get_opt tag $@) ]; then
	if [ -z ${2} ]; then
		...
		docker tag ${docker_image_local} nobetternick/pyjetty:${2}
		docker images
		exit $?
	fi
fi
